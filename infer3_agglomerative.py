...
if __name__=="__main__":
    ...
    for i, instance in enumerate(dataset_para):
        ...
        for k in [2,3,4,5]:
            ...
            sim = lambda x, y: classifier_docu.classifier_para(((x+y)/2).reshape(1, -1))[0][1]
            def sim_affinity(X):
                return pairwise_distances(X, metric=sim)

            clusterer = AgglomerativeClustering(n_clusters=k, metric=sim_affinity, linkage="complete")
